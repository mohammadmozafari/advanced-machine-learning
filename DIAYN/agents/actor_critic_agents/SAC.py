...
class SAC(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.TEPEE = self.hyperparameters["training_episodes_per_evaluation_episode"]
    ...
    def step(self):
        ...
        eval_ep = (self.episode_number + 1) % self.TEPEE == 0 and self.do_evaluation_iterations
        ...
        while not self.done:
            ...
            if not eval_ep: self.save_experience(experience=(self.state, self.action, self.reward, self.next_state, mask, self.info))
    ...
    def learn(self):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, mask_batch, info_batch = self.sample_experiences()
