...
class Base_Agent(object):
    ...
    def get_trials(self):
        ...
        if self.environment_title in ["AntMaze", "FetchReach", "Hopper", "Walker2d", "CartPole", "Ant", "MountainCar"]: return 100
    ...
    def run_n_episodes(self, num_episodes=None, save_and_print_results=True):
        ...
        while self.episode_number < num_episodes:
            self.reset_game()
            self.step()
            if save_and_print_results: self.save_and_print_result() # saves games scores as well
    ...
    def print_rolling_result(self):
        ...
        text = text.format(len(self.game_full_episode_scores), self.rolling_results[-1], self.max_rolling_score_seen,
                                     self.game_full_episode_scores[-1], self.max_episode_score_seen)
        print(self.agent_name, "> ", text)
    ...
    def save_experience(self, memory=None, experience=None):
        ...
        if experience is None: experience = self.state, self.action, self.reward, self.next_state, self.done, self.info
