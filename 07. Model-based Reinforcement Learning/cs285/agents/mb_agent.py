...
class MBAgent(BaseAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for i in range(self.ensemble_size):
            ...
            # choose a batch of observations
            indices = np.arange(0, num_data)
            np.random.shuffle(indices)
            this_ens_indices = indices[:num_data_per_ens]

            observations = ob_no[this_ens_indices]
            actions = ac_na[this_ens_indices]
            next_observations = next_ob_no[this_ens_indices]
